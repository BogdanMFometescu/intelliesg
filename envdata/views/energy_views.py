...
class EnergyListView(LoginRequiredMixin, CompanyContextMixin, FilterView):
    ...
    def get_queryset(self):
        ...
        return Energy.objects.filter(profile__user=self.request.user)

    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        filtered_qs = \
            self.filterset.qs.annotate(co2_for_energy_emissions=F('quantity') * F('emission_factor')).aggregate(
